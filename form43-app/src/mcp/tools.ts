...
export const TOOLS: ToolDef[] = [
  ...
  },
  {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        certifier_ref: { type: 'string', description: "EXTERNAL building certifier's reference number — the licensed building certifier overseeing the job, NOT Driven. Driven is the competent person (waterproofing licensee) certifying its own aspect, not the certifier. Leave blank if unknown; never invent it." },
        da_number: { type: 'string', description: 'Development approval (DA) number for the job.' },
        insp_date: { type: 'string', description: 'Date approval to inspect was received FROM the external building certifier (DD/MM/YYYY or YYYY-MM-DD).' },
        cert_date: { type: 'string', description: 'Date Driven (the competent person) certifies the aspect.' },
        notes: { type: 'string' },
      },
  ...
  },
  {
    name: 'form43_pdf',
    description:
      'Render the OFFICIAL QLD/QBCC Form 43 (Certificate of Compliance — Aspect) as a PDF, base64-encoded. Driven Waterproofing is the COMPETENT PERSON certifying its own waterproofing aspect (licensee block is pre-filled, Licence 15214278) — NOT the building certifier. The certifier_ref / insp_date fields refer to the EXTERNAL building certifier who relies on this certificate. Pass the same fields as form43_save plus optional builder_name/contact_name. Read-only — does not persist anything.',
    inputSchema: {
      type: 'object',
      properties: {
        job_id: { type: 'string' },
        ...
        products_used: { type: 'array', items: { type: 'string' } },
        areas_waterproofed: { type: 'array', items: { type: 'string' } },
        certifier_ref: { type: 'string', description: "EXTERNAL building certifier's reference number (the licensed certifier overseeing the job, NOT Driven). Leave blank if unknown; never invent it." },
        da_number: { type: 'string', description: 'Development approval (DA) number for the job.' },
        insp_date: { type: 'string', description: 'Date approval to inspect was received FROM the external building certifier.' },
        cert_date: { type: 'string', description: 'Date Driven (the competent person) certifies the aspect.' },
